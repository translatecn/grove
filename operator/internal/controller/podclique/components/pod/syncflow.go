...
import (
	...
	"github.com/ai-dynamo/grove/operator/internal/over-index"
...
func getPodCliqueExpectationsStoreKey(logger logr.Logger, operation string, pclqObjMeta metav1.ObjectMeta) (string, error) {
	...
	pclqExpStoreKey, err := over_expect.ControlleeKeyFunc(&grovecorev1alpha1.PodClique{ObjectMeta: pclqObjMeta})
	if err != nil {
...
func (r _resource) createPods(ctx context.Context, logger logr.Logger, sc *syncContext, numPods int) (int, error) {
	...
	availableIndices, err := over_index.GetAvailableIndices(logger, sc.existingPCLQPods, numPods)
	if err != nil {
